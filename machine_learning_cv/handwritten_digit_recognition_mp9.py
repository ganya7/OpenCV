...
# reshape is flattening the data
...
# knn = cv2.KNearest()  #opencv2
knn = cv2.ml.KNearest_create()
# knn.train(train, train_labels)  #opencv2
knn.train(train, cv2.ml.ROW_SAMPLE, train_labels)
# ret, result, neighbors, distance = knn.find_nearest(test, k=3)    #opencv2
...
_, contours, hierarchy = cv2.findContours(edged_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
for c in contours:
    ...
    if w >= 5 and h >= 25:  # start filtering contours from here 5px,25px
        roi = blurred[y:y + h, x:x + w]     # cropping the contour
        ret, roi = cv2.threshold(roi, 127, 255, cv2.THRESH_BINARY_INV)
        squared = makeSquare(roi)   #making a square
        final = resize_to_pixel(20, squared)    #resize to 20x20px
        cv2.imshow("final", final)
        final_array = final.reshape((1, 400))   #flattening of the image (20x20) to (1x400)
        final_array = final_array.astype(np.float32)    #converting data type for knn to float32
        # ret, result, neighbours, dist = knn.find_nearest(final_array, k=1)    #opencv2
        ret, result, neighbours, dist = knn.findNearest(final_array, k=1)  #feeding the contour in the classifier here
