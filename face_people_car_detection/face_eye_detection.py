...
# cascade classifier flow : load classifier -> pass image to classifier/detector -> get location/region of interest(ROI) for detected objects -> draw rectangle over detected objects
import numpy as np
import cv2
...
for (x, y, w, h) in faces:
    ...
    cv2.imshow('Only face detection no eye detection', img)
    cv2.waitKey(0)
    ...
    for (ex, ey, ew, eh) in eyes:
        ...
        cv2.imshow('Face and eye detection of static image', img)
        cv2.waitKey(0)
